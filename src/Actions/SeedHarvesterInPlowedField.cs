...
{
    public class SeedHarvesterInPlowedField
    {
        public static void ListResources(Farm farm, string id, string type)
        {
            IEnumerable<PlowedField> CorrectFieldEnumerable = from field in farm.PlowedFields
                                                               where field.ShortId == id
                                                               select field;

            List<PlowedField> CorrectFieldList = CorrectFieldEnumerable.ToList();

            PlowedField CorrectField = CorrectFieldList[0];

            IEnumerable<PlowedFieldReport> OrderedFlowers = (from flower in CorrectField.plantsList
            ...
            List<PlowedFieldReport> OrderedFlowersList = OrderedFlowers.ToList();
            ...
            Console.WriteLine();
            foreach (PlowedFieldReport flower in OrderedFlowersList)
            {
